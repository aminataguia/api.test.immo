...
@app.get("/revenu_fiscal_moyen/", description='Retourne le revenu fiscal moyen de la ville donnée')
async def revenu_fiscal_moyen(year: str = Depends(validate_year), city: str = ""):
    try:
        con, cur = connexion_database()

        req = f"SELECT revenu_fiscal_moyen, date, ville FROM foyers_fiscaux WHERE date = {year} AND ville = '{city}'"
        cur.execute(req)
        result = cur.fetchall()
        if result is None or len(result)==0:
            return HTTPException(status_code=400, detail="Aucune valeur trouvée")
        elif len(result)==1 :
            return result[0][0]
        else: return result
        ...
        fermeture_database(con)

    finally:
        fermeture_database(con)
#Inclusion des termes try,except,else et finally...
